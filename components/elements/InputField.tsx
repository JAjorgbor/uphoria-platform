...
const InputField: FC<InputFieldProps> = ({
  ...
  const baseClass = `w-full p-3 block text-nevada w-full rounded-lg !outline-[0.5px] outline-gray-500 ${
    startContent ? "rounded-l-none outline-gray-500 outline-l-none" : ""
  } ${endContent ? "rounded-r-none outline-gray-500 outline-r-none" : ""}  ${
    !!errorMessage
      ? "!bg-red-100 outline-red-500"
      : " !bg-transparent focus:!bg-transparent outline focus:!outline-rose-800 "
        ...
        return (
          <>
            ...
            >
              <ListboxButton
                disabled={disabled}
                className="group relative border flex justify-between items-center w-full focus:border-rose-800 rounded-lg p-2 py-3"
                ref={listBoxButtonRef}
              >
                {options?.find((each) => each.value == value)?.label ||
                  "Select..."}
                <FaChevronDown className="text-rose-800" />
              </ListboxButton>
              <ListboxOptions
                anchor="bottom"
              ...
              >
                {placeholder && (
                  <ListboxOption
                    disabled
                    value=""
                    className="group flex items-center gap-2 rounded-lg py-1.5 px-3 bg-transparent select-none data-[focus]:bg-rose-800 data-[focus]:text-white hover:bg-rose-800 hover:text-white cursor-pointer"
                  >
                ...
                {options?.map((item) => (
                  <ListboxOption
                    key={item.value}
                    value={item}
                    className="group flex items-center gap-2 rounded-lg py-1.5 px-3 text-rose-800 bg-white select-none data-[focus]:bg-rose-800 data-[focus]:text-white hover:bg-rose-800 hover:text-white cursor-pointer"
                  >
                    <FiCheck className="invisible size-4 group-data-[selected]:visible" />
        ...
        return (
          <>
            <Combobox value={value} onChange={onChange}>
              <div
                className="relative border focus:border-rose-800 rounded-lg p-2 w-full"
                tabIndex={-1}
      ...
      case "checkbox":
        return (
          <Checkbox
            ...
            className="p-0.5 rounded-md border cursor-pointer text-transparent data-[checked]:text-white data-[checked]:bg-rose-800"
            disabled={disabled}
            color="secondary"
  ...
  return (
    <div className={`space-y-1 ${className} `}>
      ...
      >
        ...
        type == "switch" ||
        renderLabelRight ? (
          <p className={`order-2 text-rose-800 ${classNames.label}`}>
            {label}{" "}
            {isRequired && <span className="text-red-700 text-xl">*</span>}
          </p>
        ) : renderLabelLeft ? (
          <p className={`order-1 text-rose-800 ${classNames.label}`}>
            {label}{" "}
            {isRequired && <span className="text-red-700 text-xl">*</span>}
          </p>
        ) : (
          <p>
            <span className={`text-rose-800 ${classNames.label}`}>{label}</span>
            {isRequired && <span className="text-red-700 text-xl">*</span>}
          </p>
        )}
